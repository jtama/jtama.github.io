...
Ou _recipe_ dans la langue de Shakespear est l'élément atomique pour travailler sur le code. Elle peut avoir des pré-conditions, lui permettant de savoir si elle doit traiter un fichier source.

Elle peut comprendre un _visitor_ qui permet d'effectuer ou non des modifications sur le code source.

Mais elle peut aussi avoir une liste de recettes qui s'appliqueront séquentiellement. Ce qui signifie que la façon d'implémenter un refactoring complexe est de composer des _recettes_, et à la fin, cela s'appelle aussi une recette.
