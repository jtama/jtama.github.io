...
:description: Arrêtez de penser au comment et préoccupez-vous du quoi !
:page-tags: Tutoriels
:page-image: jreleaser-duke.webp
:page-author: jtama
...
Publier un projet ce n'est pas seulement pousser un paquet vers un dépôt centralisé. Cela peut aussi signifier signer et générer les artefacts, produire un _SBOM_, assembler et publier un _changelog_, annoncer la publication sur votre _hub_ git favori, et même faire le dire sur Zulip, Bluesky et encore bien d'autres choses.

image:jreleaser-workflow.webp[Flux de travail JReleaser, link=https://jreleaser.org/guide/latest/concepts/workflow.html,window=_blank]
...
Comme _Maven Central_ est le but ultime pour les développeurs Java, et comme c'est aussi l'un des dépôts les plus compliqués, c'est celui que nous examinerons dans cet article de blog. Mais bien sûr, une fois que vous aurez fait cela, tout le reste sera du gâteau 🧁 !
...
Connectez-vous à https://central.sonatype.com/[maven central], cliquez sur l'élément de menu en haut à droite "View User Tokens", et générez un nouveau jeton.

Vous pouvez spécifier son nom et sa date d'expiration. Personnellement, j'ai choisi de faire une folie pour celui-ci :

image::sonatype_token.webp[Le nom du jeton est JReleaser]

=== Installer JReleaser

Il existe de nombreuses façons https://jreleaser.org/guide/latest/install.html[d'installer JReleaser,window=_blank], mais mon choix pour mon environnement de dev est d'utiliser https://www.jbang.dev/[JBang,window=_blank] :
...
----
<1> [[local-deployment-repository]]Vous pouvez bien sûr changer cette valeur
====
...
<3> Où trouver les artefacts localement. Doit correspondre à <<local-deployment-repository, la valeur utilisée dans votre configuration de projet>>.
...
https://jreleaser.org/guide/latest/reference/environment.html#_env_files[Pour en savoir plus à ce sujet].
...
----
<1> TOML supporte les chaînes multilignes utilisant des triples _quotes_ : `"""`
<2> Je fais seulement des "draft release" depuis l'environnement de dev
<3> Valeurs obtenues dans le chapitre <<obtaining-your-maven-central-credentials>>. Ce sont les mêmes pour *maven central* et *nexus2_snapshot*

Une dernière chose, je préfère faire uniquement des _dry run_ localement, mais cela ne peut pas être spécifié via un fichier pour le moment, donc j'exporte la valeur juste avant de lancer les commandes:
...
Une fois que vous êtes satisfait, vous pouvez éventuellement désactiver la propriété dry run et relancer, mais je préfère faire la vraie publication depuis ma _**CI**_, qui est Github image:github-icon.svg[Icône Github,20,16] pour le moment.
...
Nous n'avons fait qu'effleurer la surface de ce qui peut être fait avec JReleaser. Si vous voulez un exemple vraiment complet, https://jreleaser.org/guide/latest/examples/jreleaser.html[voici comment JReleaser se publie lui-même], avec JReleaser bien sûr.
